...
function mockPrototype(win) {
	...
	*/
	...
	const getContext = Reflect.get(htmlCanvasElementPrototype, "getContext");
	if (!vitest.vi.isMockFunction(getContext)) getContext2D.internal = getContext;
	else getContext2D.internal = getContext.internal;
	...
	*/
	const toBlobOverride = vitest.vi.fn(function toBlobOverride(callback, mimetype) {
	...
	const toBlob = Reflect.get(htmlCanvasElementPrototype, "toBlob");
	if (!vitest.vi.isMockFunction(toBlob)) toBlobOverride.internal = toBlob;
	else toBlobOverride.internal = toBlob.internal;
	...
	*/
	...
	const toDataURL = Reflect.get(htmlCanvasElementPrototype, "toDataURL");
	if (!vitest.vi.isMockFunction(toDataURL)) toDataURLOverride.internal = toDataURL;
	else toDataURLOverride.internal = toDataURL.internal;
...
function setupVitestCanvasMock(window) {
	window_default(window || globalThis.window);
}
//#endregion
