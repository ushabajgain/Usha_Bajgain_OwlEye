...
const MyTickets = () => {
    ...
    const { tickets: liveSyncTickets } = useSafety();
    ...
    // Sync live updates from context into the local bookings display
    React.useEffect(() => {
        if (Object.keys(liveSyncTickets).length === 0) return;
        
        setBookings(prev => prev.map(pkgTicket => {
            const live = liveSyncTickets[pkgTicket.fullId];
            if (live) {
                return {
                    ...pkgTicket,
                    status: live.status === 'issued' ? 'Confirmed' : live.status === 'scanned' ? 'Confirmed' : 'Cancelled'
                };
            }
            return pkgTicket;
        }));
            ...
            <main style={s.main}>
                ...
                <div style={s.content}>
                    ...
                    <div style={s.statsRow}>
                        <StatCard label="My Tickets" value={bookings.length} icon={Ticket} color={ACCENT} />
                        <StatCard label="Total Spent" value={`Rs. ${bookings.reduce((sum, b) => sum + parseFloat(b.amount), 0).toLocaleString()}`} icon={DollarSign} color="#ec4899" />
                        <StatCard label="Active Events" value={[...new Set(bookings.map(b => b.event))].length} icon={CalendarIcon} color="#a855f7" />
                    </div>
                    ...
                    <div style={s.tableCard}>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                                <tr>
                                    {['Invoice ID', 'Date', 'Name', 'Event', 'Ticket Category', 'Price', 'Qty', 'Amount', 'Status', 'Actions'].map(h => (
